...
function initSketchpad() {
    ...
    var canvasProjector = document.querySelector("#canvasProjector ")
    //HIDE LOGO
    var logoElem = document.querySelector('[title="sketchpad.pro library is available under AGPL-3 license, visit http://developers.sketchpad.pro for source code. This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY."]');
    ...
    //SCREENSHOT
    document.getElementById('tool-screenshot').addEventListener("click", function (el) {
...
async function project() {
    ...
    var dataURL = await canvas.toDataURL('image/jpeg', 1.0);
    imgConverted.src = await dataURL
    BoardMat.albedoTexture = await new BABYLON.Texture(imgConverted.src, scene);
    BoardMat.albedoTexture.vScale = await -1
}